...
export type BitmapResult = { version: string; width: number; height: number; inputDigest: string; pixelDigest: string; packed: string; proof: string; zpl: string };
...
let cacheBytes = 0;
export async function getBitmapProof(template: LabelTemplate, format: LabelFormat, values: FeedValues = {}): Promise<BitmapResult> {
  const key = JSON.stringify([template.elements, template.thermalRenderMode, format, values]);
  const found = cache.get(key); if (found) return found;
  const request = fetch('/api/thermal/render', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ template, format, feeds: [values] }) })
    .then(async response => {
